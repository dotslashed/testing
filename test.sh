...
echo "collecting domomains from urls"

sleep 3

echo "[+]printing and saving the page titles..."
...
go get -u github.com/nytr0gen/deduplicate

echo "[+]Gathering only paths from urls..."
...
echo "Looking for DOM xss from js files"

sleep 3

cat final_subs.txt | getJS | grep $TARGET | httpx -silent --match-regex "addEventListener\((?:'|\")message(?:'|\")" >> dom1.txt

go get github.com/hakluke/hakrawler

cat final_subs.txt | hakrawler -d 3 -subs -u | httpx -silent --match-regex "addEventListener\((?:'|\")message(?:'|\")" >> dom2.txt

cat dom1.txt dom2.txt | sort -u >> domxss.txt


rm dom1.txt dom2.txt

echo "[+]SSRF testing..."

echo "Enter your collaborator payload:::"
...
echo "Job done!"
echo "Don't worry saved everything!!!"
echo "Please do other tasks manually!!!"

echo "Please wait for js links and secrets:::"
...
cat jsurls.txt | while read urls do; do echo $urls && python3 linkfinder.py -i $urls -o cli ;done >> js_internal_links.txt
...
cat jsurls.txt | while read urls do; do python3 SecretFinder.py -i $urls -o cli ;done >> js_internal_secrets.txt
